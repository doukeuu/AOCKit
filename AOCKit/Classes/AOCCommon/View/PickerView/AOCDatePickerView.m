...
#import "AOCDatePickerView.h"
#import "AOCDefineSize.h"
#import "AOCDefineColor.h"

@interface AOCDatePickerView () <UIPickerViewDataSource, UIPickerViewDelegate>
{
...
@implementation AOCDatePickerView

// 展示日期选择视图
+ (void)showDatePickerWithType:(AOCDatePickerType)type completion:(DatePickerBlock)block {
...
+ (void)showPickerWithType:(AOCDatePickerType)type
    ...
    AOCDatePickerView *picker = [[AOCDatePickerView alloc] initWithFrame:frame];
    picker.pickerType = type;
    picker.minimumDate = minDate;
    picker.maximumDate = maxDate;
    picker.includeTotal = includeTotal;
    picker.block = block;
    picker.animationType = AOCAnimationPopFromBottom;
...
- (void)commonInitialize {
    _lineStroked = NO;
    _pickerType = AOCDatePickerYear | AOCDatePickerMonth | AOCDatePickerDay;
    _yearColumn = 0;
    _monthColumn = 1;
    _dayColumn = 2;
    _totalColumn = 3;
...
- (void)generateSubviews {
    ...
    // 背景框视图
    CGFloat bezelH = SCREEN_FIT_H(280);
    ...
    [cancelButton setTitleColor:kCOLOR_000 forState:UIControlStateNormal];
    ...
    [determineButton setTitleColor:kCOLOR_000 forState:UIControlStateNormal];
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    if (self.block) self.block(-1, -1, -1);
...
// 日期类型
- (void)setPickerType:(AOCDatePickerType)pickerType {
    _pickerType = pickerType;
    _totalColumn = 0;
    if (_pickerType & AOCDatePickerYear) {
        _yearColumn = _totalColumn;
        _totalColumn ++;
    }
    if (_pickerType & AOCDatePickerMonth) {
        _monthColumn = _totalColumn;
        _totalColumn ++;
    }
    if (_pickerType & AOCDatePickerDay) {
...
//        firstLine.backgroundColor = COLOR_WHITE;
//        secondLine.backgroundColor = COLOR_WHITE;
...
//        _downLine.frame = downFrame;
    ...
    titleLabel.textColor = kCOLOR_000;
...
// 重置月份到一月
- (void)resetMonth {
    if (!(_pickerType & AOCDatePickerMonth)) return;
...
// 重置日期到一号
- (void)resetDay {
    if (!(_pickerType & AOCDatePickerDay)) return;
