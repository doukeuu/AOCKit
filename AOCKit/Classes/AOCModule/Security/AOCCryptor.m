...
static const char* encryptWithKeyAndType(const char *text, CCOperation encryptOperation, char *key) {
    ...
    if (encryptOperation == kCCDecrypt) { //传递过来的是decrypt 解码
        //解码 base64
        NSData *originData = [textString dataUsingEncoding:NSUTF8StringEncoding];
        NSData *decryptData = [originData base64EncodedDataWithOptions:NSDataBase64Encoding64CharacterLineLength]; // [GTMBase64 decodeData:[textString dataUsingEncoding:NSUTF8StringEncoding]];//转成utf-8并decode
    ...
    if (encryptOperation == kCCDecrypt) { //encryptOperation==1  解码
        ...
        //编码 base64
        NSData *data = [NSData dataWithBytes:(const void *)dataOut length:(NSUInteger)dataOutMoved];
        result = [data base64EncodedStringWithOptions:NSDataBase64Encoding64CharacterLineLength]; //[GTMBase64 stringByEncodingData:data];
    }
