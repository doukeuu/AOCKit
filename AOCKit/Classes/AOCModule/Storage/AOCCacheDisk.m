...
- (instancetype)initWithCachePath:(NSString *)cachePath config:(nonnull AOCCacheConfig *)config {
    if (self = [super init]) {
...
- (void)removeExpiredData {
    ...
    switch (self.config.diskCacheExpireType) {
        case AOCCacheConfigExpireTypeAccessDate:
            cacheContentDateKey = NSURLContentAccessDateKey;
            break;
        case AOCCacheConfigExpireTypeModificationDate:
            cacheContentDateKey = NSURLContentModificationDateKey;
            break;
        case AOCCacheConfigExpireTypeCreationDate:
            cacheContentDateKey = NSURLCreationDateKey;
            break;
        case AOCCacheConfigExpireTypeChangeDate:
...
- (nullable NSString *)cachePathForKey:(nullable NSString *)key inPath:(nonnull NSString *)path {
    NSString *filename = AOCDiskCacheFileNameForKey(key);
...
#define AOC_MAX_FILE_EXTENSION_LENGTH (NAME_MAX - CC_MD5_DIGEST_LENGTH * 2 - 1)

#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
static inline NSString * _Nonnull AOCDiskCacheFileNameForKey(NSString * _Nullable key) {
    ...
    if (ext.length > AOC_MAX_FILE_EXTENSION_LENGTH) {
        ext = nil;
    }
